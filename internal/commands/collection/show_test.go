...
func TestNewShowCmd(t *testing.T) {
	...
	if cmd.Use != "show COLLECTION_ID" {
		t.Errorf("NewShowCmd() Use = %q, want %q", cmd.Use, "show COLLECTION_ID")
	}
...
func TestRunShow_NoToken(t *testing.T) {
	...
	err := runShow(ctx, "nonexistent-profile-test", "text", "test.example.org", "test-collection-id", buf)
	if err == nil {
