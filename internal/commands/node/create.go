...
// NewCreateCmd creates the node create command.
func NewCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Create a new node on the endpoint.

A node represents a compute resource that can participate in data transfers.
Nodes can be configured to accept incoming transfers, outgoing transfers, or both.
...
// runCreate executes the node create command.
func runCreate(ctx context.Context, profile, formatStr, endpointFQDN string,
	...
	// Create node
	created, err := gcsClient.CreateNode(ctx, node)
	if err != nil {
	...
	// Text format
	if err := formatter.Println("Node created successfully!"); err != nil {
		return err
	}
