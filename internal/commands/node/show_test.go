...
func TestNewShowCmd(t *testing.T) {
	...
	if cmd.Use != "show NODE_ID" {
		t.Errorf("NewShowCmd() Use = %q, want %q", cmd.Use, "show NODE_ID")
	}
...
func TestRunShow_NoToken(t *testing.T) {
	...
	err := runShow(ctx, "nonexistent-profile-test", "text", "test.example.org", "test-node-id", buf)
	if err == nil {
